...
def aplicarBinarizacion(ind, transferFunction, binarizationFunction, bestSolutionBin, indBin, iter, pop, maxIter, pos_ind, chaotic_map):
    ...
    if not es_binario_np(bestSolutionBin):
        print("bestSolutionBin no es binario")
    ...
    if not es_binario_np(individuoBin):
        print("individuoBin no es binario")
...
def Standard(step1):
    rand = random.uniform(0.0, 1.0)
    return np.where(step1 >= rand, 1, 0)
